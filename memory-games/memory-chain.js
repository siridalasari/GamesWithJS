...
function displayResult(userGuess) {
  ...
  const stringCards = GENERATED_CARDS.join();
  const stringUserGuess = userGuess.join();
  ...
  if (!hasGuessed) {
    console.log(alignCenterHorizontally("Displayed cards:" + GENERATED_CARDS.join("  ")));
    console.log(alignCenterHorizontally("Entered cards  :" + userGuess.join("  ")));
  }
...
function takeUserInput(inputLength = 8) {
  ...
  return displayResult(userInput);
}
...
function displayCards() {
  console.clear();
  console.log("sirisha");
  console.log(styles("emptyLine", 18) + alignCenterHorizontally(GENERATED_CARDS.join("   ")));
...
function gameStartAnimation() {
  ...
  return countDown([number3Block(), number2Block(), number1Block()]);
}
...
resultBoard("👏", "hello this sirisha");
